...
def upload_pic():
    path=os.path.join(os.path.abspath('.'),'app/static')
    print('you upload we or teach? w or t')
    choice=raw_input('You choose:')
    if choice=='t':
        path=os.path.join(path,'teach')
        max_uni=0
        if os.listdir(path):
            max_uni=max(os.listdir(path))
            max_uni=int(max_uni[:-4])+1
        os.system('rz')
        os.system('mv *.jpg %s/%s.jpg'%(path,max_uni))
    elif choice=='w':
        path=os.path.join(path,'we')
        max_uni=0
        if os.listdir(path):
            max_uni=max(os.listdir(path))
            max_uni=int(max_uni[:-4])+1
...
if __name__ == '__main__':
    fnList = [update_config, update_articles, check_for_upload, upload_to_server,upload_pic]
    while 1:
        print('*** Command List ***\n0: Quit')
